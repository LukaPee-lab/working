...
- 비어 있는 노드 슬롯을 우클릭하고 `연구 노드 추가`를 누르면 선택한 `STEP / row` 좌표에 정확히 새 노드와 연결 효과를 만듭니다. 주변 노드의 연결은 자동으로 바꾸지 않습니다.
...
- 마우스 휠은 확대/축소, 우클릭 드래그는 이동입니다. Scene의 빈 곳에서 좌클릭 드래그하면 선택 사각형과 겹친 STEP 블록을 한꺼번에 선택합니다. `Ctrl+클릭`으로 STEP 선택을 더하거나 뺄 수도 있습니다.
