...
- 노드를 한 개 선택하면 카드 옆에 `빠른 노드 편집` 창이 열립니다. `노드 설정` 탭에서는 자주 바꾸는 `image`, `active_item_id`, `active_item_value`, `active_step`을 수정하고, `효과 설정` 탭에서는 연결된 `nexus_effect`의 Export ID, 메모, 타입, 조건, 값과 나머지 원본 칼럼을 바로 수정합니다. 연결 효과가 없는 노드는 같은 탭에서 효과 행을 생성할 수 있으며, Inspector에는 전체 칼럼이 계속 표시됩니다.
